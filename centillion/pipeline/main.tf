...
resource "azurerm_resource_group" "jeevaCreation" {
  name     = "jeevaCreation"
...
resource "azurerm_network_security_group" "snet" {
  name                = "snet"
  location            = azurerm_resource_group.jeevaCreation.location
  resource_group_name = azurerm_resource_group.jeevaCreation.name
}
...
resource "azurerm_storage_data_lake_gen2_filesystem" "example_fs" {
  name               = "jeevastorage"  # Change to a unique name
...
resource "azurerm_data_factory" "example" {
  name                = "jeevaCreation-data-factory"
  location            = azurerm_resource_group.jeevaCreation.location
  resource_group_name = azurerm_resource_group.jeevaCreation.name
...
resource "azurerm_data_factory_pipeline" "example" {
  name               = "jeevaCreation-data-pipeline"
